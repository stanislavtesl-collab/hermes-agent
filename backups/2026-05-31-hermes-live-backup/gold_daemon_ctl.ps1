...
function Get-DaemonProcs {
  Get-CimInstance Win32_Process | Where-Object {
    $_.CommandLine -match 'gold_manager_daemon\.py' -and
    ($_.Name -eq 'python.exe' -or $_.Name -eq 'pythonw.exe')
  }
}


function Is-PidAlive([int]$TargetPid) {
  try {
...
if($Action -eq 'dedupe'){
  $h=HeartbeatInfo
  $keep=$null
  if($h -and $h.age -le 120 -and (Is-PidAlive -TargetPid $h.pid)){ $keep=$h.pid }
...
if($Action -eq 'start'){
  ...
  $h=HeartbeatInfo
  if($h -and $h.age -le 60 -and (Is-PidAlive -TargetPid $h.pid)){
