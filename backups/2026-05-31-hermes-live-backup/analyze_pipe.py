...
def fetch_bars(source, symbol, interval, count):
    ...
    if source == "mt5":
        ...
        td_symbol = (symbol or "").strip().upper()
        # Twelve Data expects FX/metals pairs as BASE/QUOTE.
        ...
        cmd = [py, r"C:\Users\Administrator\twelvedata_query.py", "bars", td_symbol, interval, str(count)]
    timeout_s = _safe_int(os.getenv("MT5_QUERY_TIMEOUT_SEC", "18"), 18) if source == "mt5" else 35
    data = _run_json(cmd, timeout=max(8, timeout_s))
    bars = data.get("bars", [])
    if not bars:
