...
def update_job(job_id: str, updates: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    for i, job in enumerate(jobs):
        ...
        # Normalize repeat in updates so all callers converge on the same
        # storage shape. Supports legacy payloads:
        #   repeat=-1 / 0  -> forever (times=None)
        #   repeat=5       -> {"times": 5, "completed": <preserved>}
        #   repeat={"times": -1} -> {"times": None, ...}
        if "repeat" in updates:
            existing_repeat = job.get("repeat") if isinstance(job.get("repeat"), dict) else {}
            existing_completed = existing_repeat.get("completed", 0)
            try:
            ...
            raw_repeat = updates.get("repeat")
            if isinstance(raw_repeat, dict):
                raw_times = raw_repeat.get("times")
                raw_completed = raw_repeat.get("completed", existing_completed)
                try:
                    norm_completed = int(raw_completed)
                except Exception:
                    norm_completed = existing_completed
                if norm_completed < 0:
                    norm_completed = 0
            else:
                raw_times = raw_repeat
                norm_completed = existing_completed

            norm_times = None
            if raw_times is not None:
                try:
                    norm_times = int(raw_times)
                except Exception:
                    norm_times = None
                if norm_times is not None and norm_times <= 0:
                    norm_times = None

            updates["repeat"] = {"times": norm_times, "completed": norm_completed}
