...
## ⚡ GOLD Scalping (Nikolai's FxPro) — Customization

**Счёт:** 591712391 (FxPro Demo, Mykola Ievtushenko)
**Инструмент:** GOLD (символ MT5), лот 0.03
**Торговый журнал:** `C:\Users\Administrator\Desktop\FxPro\trade_journal.md`
**Скрипт-парсер логов:** `C:\Users\Administrator\Desktop\FxPro\scripts\update_journal.py`

**CRITICAL RULE:** Журнал содержит ТОЛЬКО живые сделки с демо-счёта MT5. Бэктесты и исторические симуляции НЕ добавлять — пользователь явно это указал 31 мая 2026.

**Скрипт `update_journal.py`:** парсит `.gold_manager.log` и `.gold_executor.log`, находит новые закрытые сделки по тикетам, добавляет их в markdown-журнал. Запуск:
```
python C:\Users\Administrator\Desktop\FxPro\scripts\update_journal.py        # добавить новые
python C:\Users\Administrator\Desktop\FxPro\scripts\update_journal.py stats  # статистика
```

После каждой торговой сессии запускать скрипт для актуализации журнала. Можно автоматизировать через cron.
