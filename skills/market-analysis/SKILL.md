...
## Data Source Priority (GOLD/XAUUSD)

When the user asks for GOLD analysis, prefer this data source order:

1. **MT5 (MetaTrader 5)** — best priority. Direct terminal data, real-time, no API limits.
   - Caveat: if executor already holds MT5 connection, `mt5.copy_rates_from_pos()` may return None.
   - Workaround: `mt5.symbol_info_tick('GOLD')` still works for BID/ASK price
   - `mt5.initialize(path=path)` — only one process can init the terminal

2. **yfinance (GC=F futures)** — fallback when MT5 unavailable.
   - GC=F ≈ spot + $28-30 spread (futures premium)
   - `period` must be one of: 1d, 5d, 1mo, 3mo, 6mo, 1y, 2y, 5y, 10y, ytd, max (NOT '2h' or '4h')
   - Can have multi-level column headers → flatten with `df.columns = [c[0] for c in df.columns]`
   - Good for H1/D1 analysis, not for M1/M5 real-time scalping

3. **Twelve Data API** — last resort. Free tier: 800 requests/day. Easy to exhaust.
   - Returns newest-first → always `.reverse()` the values array
   - Error 429 when limit reached

4. **DO NOT USE** for GOLD: Yahoo Finance spot pairs (XAUUSD=X) — unreliable for metals
