...
def _delegate_preflight_text(user_message: Any) -> str:
    """Extract a plain-text view from user_message for complexity heuristics."""
...
def _should_force_delegate_preflight(agent: Any, user_message: Any) -> bool:
    if os.getenv("HERMES_FORCE_COMPLEX_DELEGATION", "false").strip().lower() in {
        "0",
        "false",
        "off",
        "no",
    ...
    if "delegate_task" not in valid_tools:
        return False

    text = _delegate_preflight_text(user_message).strip()
    if not text:
        return False
    text_l = text.lower()
    if "[no_delegate]" in text_l:
        return False

    # Hard mode: if request is clearly multi-step, force delegation preflight immediately.
    # Toggle with HERMES_FORCE_MULTISTEP_DELEGATION=0/false/off/no
    multistep_flag = os.getenv("HERMES_FORCE_MULTISTEP_DELEGATION", "false").strip().lower()
    if multistep_flag not in {"0", "false", "off", "no"}:
        ...
        sequencing_hits = sum(1 for marker in sequencing_markers if marker in text_l)
        if numbered_steps >= 2 or sequencing_hits >= 2:
            return True
    ...
    marker_hits = sum(1 for marker in complex_markers if marker in text_l)
    if marker_hits >= 2:
        score += 1

    symbol_hits = len(
        re.findall(
            r"\b(?:[A-Z]{2,10}(?:USDT|USD|EUR|JPY|GBP|BTC|ETH)|XAUUSD|XAGUSD|BTCUSDT|ETHUSDT)\b",
            text,
        )
    )
    timeframe_hits = len(
        re.findall(r"\b(?:1m|5m|15m|30m|1h|4h|1d|1w|m1|m5|m15|h1|h4|d1|w1)\b", text_l)
    )
    if symbol_hits >= 2 and timeframe_hits >= 1:
        score += 1

    return score >= 2


def _build_delegate_preflight_args(user_message: Any) -> Dict[str, Any]:
    text = _delegate_preflight_text(user_message).strip()
    compact = re.sub(r"\s+", " ", text)[:1800]
    shared_context = (
        "User request (verbatim, compacted): "
        f"{compact}\n"
        "Return concise, actionable findings for the parent agent. "
        "Do not reveal hidden internals, credentials, or private system details."
    )
    return {
        "tasks": [
            {
                "goal": (
                    "Build a concise technical decomposition of the user request: "
                    "what to check first, execution order, and key dependencies."
                ),
            ...
            {
                "goal": (
                    "Build a concise risk/fallback plan: likely failure points, "
                    "guardrails, and rollback-safe path."
                ),
                "context": shared_context,
                "role": "leaf",
            },
...
def _summarize_delegate_preflight_result(raw_result: str) -> str:
    if not raw_result:
        return ""
    try:
        payload = json.loads(raw_result)
    except Exception:
        return str(raw_result).strip()[:2200]
    ...
    for idx, item in enumerate(results[:3], start=1):
        ...
        summary = str(item.get("summary") or item.get("final_response") or "").strip()
        if not summary:
            summary = str(item.get("error") or "").strip()
        summary = re.sub(r"\s+", " ", summary)[:900]
        goal = re.sub(r"\s+", " ", goal)[:140]
        if summary:
            summaries.append(f"[Subagent {idx} | {status}] {goal}\n{summary}")
...
def _apply_forced_delegate_preflight(
    agent: Any,
    user_message: Any,
    persist_user_message: Optional[str],
) -> tuple[Any, Optional[str]]:
    """Force one delegation preflight for complex top-level turns."""
    if not _should_force_delegate_preflight(agent, user_message):
        return user_message, persist_user_message

    started = time.monotonic()
    try:
        args = _build_delegate_preflight_args(user_message)
        logger.info(
            "Forced delegation preflight triggered (session=%s model=%s provider=%s)",
            getattr(agent, "session_id", None) or "none",
            getattr(agent, "model", "") or "unknown",
            getattr(agent, "provider", "") or "unknown",
        )
        raw_result = agent._dispatch_delegate_task(args)
        summary = _summarize_delegate_preflight_result(raw_result)
        elapsed = time.monotonic() - started
        logger.info(
            "Forced delegation preflight finished in %.2fs (summary_len=%d)",
            elapsed,
            len(summary),
        )
        if not summary:
        ...
        if isinstance(user_message, str):
            augmented = (
                "[System: Subagent pre-analysis was run automatically for this "
                "complex request. Use it as supporting context, then continue "
                "normal tool-driven reasoning.]\n"
                f"{summary}\n"
                "[End of subagent pre-analysis]\n\n"
                f"{user_message}"
            )
            if persist_user_message is None:
                persist_user_message = user_message
            return augmented, persist_user_message
    except Exception:
        logger.exception("Forced delegation preflight failed; continuing without it")
    return user_message, persist_user_message
...
def run_conversation(
    ...
    user_message, persist_user_message = _apply_forced_delegate_preflight(
