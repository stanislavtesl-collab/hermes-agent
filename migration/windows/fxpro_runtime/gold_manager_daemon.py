"""GOLD DAEMON v9 — PARTIAL CLOSE + ТРЕЙЛИНГ
- Авто-реконнект MT5 (3 retry)
- Heartbeat каждые 30 сек (.gold_heartbeat.json)
- PID-файл + watchdog
- Трейлинг 80/100/80 + Partial Close 50%@100pts
- Пишет в logfile каждую итерацию"""
...
last_ticket = None
trail_level = 0
partial_closed = False  # был ли partial close для текущего тикета
last_heartbeat = 0
daemon_lock_fd = None
    ...
    for attempt in range(MAX_RETRIES):
        ...
        try:
            if mt5.initialize(path=path, timeout=20000):
                time.sleep(0.3)  # дать MT5 прогреться
                return True
        except Exception as e:
            log(f"⚠️ MT5 init exception: {e}")
...
def check():
    ...
    if pos and len(pos) > 0:
        for p in pos:
            ...
            # PARTIAL CLOSE: при +100pts закрыть 50% объёма (если ещё не закрывали)
            if not partial_closed and vol >= 0.03 and pts >= 100:
                half_vol = round(vol / 2 * 1000) / 1000  # 0.03/2=0.015, без banker's rounding
                close_request = {
                    "action": mt5.TRADE_ACTION_DEAL,
                    "symbol": symbol,
                    "volume": half_vol,
                    "type": mt5.ORDER_TYPE_SELL if tp_dir == "B" else mt5.ORDER_TYPE_BUY,
                    "position": t,
                    "price": cur,
                    "deviation": 20,
                    "magic": 123456,
                ...
                if close_result and close_result.retcode == mt5.TRADE_RETCODE_DONE:
                    partial_closed = True
                    fix_pts = round((cur - entry) / 0.01, 1) if tp_dir == "B" else round((entry - cur) / 0.01, 1)
                    fix_profit = round(half_vol * 10 * abs(close_result.price - entry), 2)  # точный профит
                    write_state("partial", {
                        "title": f"✂️ PARTIAL #{t}: +{fix_pts}pts",
                        "body": f"Закрыто {half_vol} лота @ {cur:.2f} | Осталось {half_vol} в трейлинге"
                    })
                    log(f"✂️ #{t} PARTIAL CLOSE {half_vol} @ {cur:.2f} fix=+{fix_pts}pts")
                else:
                    log(f"⚠️ Partial close failed retcode={close_result.retcode if close_result else 'None'}")
    ...
    has_position = bool(pos and len(pos) > 0)
    mt5.shutdown()
    return has_position
...
log(f"🚀 GOLD DAEMON v9 STARTED PID={os.getpid()}")
log(f"   PARTIAL CLOSE | 50%@100pts | Трейлинг Activate@{TRAIL_ACTIVATE} Step@{TRAIL_STEP} Offset@{TRAIL_OFFSET}")
log(f"   HEARTBEAT every {HEARTBEAT_INTERVAL}s")
write_state("ready", {"title": "🟢 Демон v9 запущен", "body": "Partial Close 50%@100pts + трейлинг. Жду сделок."})

# Main loop
while True:
    sleep_for = NO_TRADE_SLEEP
    try:
        in_trade = check()
        sleep_for = IN_TRADE_SLEEP if in_trade else NO_TRADE_SLEEP
    except Exception as e:
