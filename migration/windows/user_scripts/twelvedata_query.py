...
def _resolve_api_key():
    direct = os.getenv("TWELVEDATA_API_KEY", "").strip()
    if direct:
        return direct
    # Local fallback for direct script invocation outside gateway env loading.
    candidates = [
        r"C:\Users\Administrator\AppData\Local\hermes\.env",
        os.path.join(os.path.dirname(__file__), ".env"),
    ]
    for candidate in candidates:
        val = _read_env_key_from_file(candidate, "TWELVEDATA_API_KEY")
        if val:
            return val
    return ""
